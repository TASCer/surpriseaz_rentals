...
def parcel_changes(parcel_changes: object):
	...
	parcel_changes_style: Styler = parcel_changes.style.set_table_styles(
		styles.get_style_changes()).set_caption(parcel_changes_caption).hide(axis='index')
...
def financials(community_avg_prices) -> None:
	...
	finance_style: Styler = community_avg_prices.style.set_table_styles(
		styles.get_style_finance()).set_caption(finance_caption).hide(axis='index')
