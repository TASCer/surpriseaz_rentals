...
def get_average_sale_price():
	...
	try:
		engine: Engine = create_engine(f'mysql+pymysql://{DB_USER}:{DB_PW}@{DB_HOSTNAME}/{DB_NAME}')

	except (AttributeError, exc.SQLAlchemyError, exc.OperationalError, exc.ProgrammingError) as e:
		logger.critical(e)
		engine: Engine = None

	with engine.connect() as conn, conn.begin():
		try:
			all_sales_ytd: DataFrame = pd.read_sql(f"""SELECT 
				p.COMMUNITY,
				o.SALE_DATE,
				o.SALE_PRICE
				# AVG(s.SALE_PRICE)
	...
	all_community_sales_ytd.to_csv(f"{my_secrets.csv_finance_path}all_ytd_community_sales.csv")
	
	community_sold_count: NDFrame = all_community_sales_ytd.groupby('COMMUNITY').count()  
	community_sold_count: NDFrame = community_sold_count.rename(columns={"SALE_DATE": "#Sold"})
	
	ytd_avg_price: DataFrame = all_community_sales_ytd.groupby(['COMMUNITY']).mean(['SALE_PRICE'])

	ytd_avg_price: DataFrame = ytd_avg_price.rename(columns={"SALE_PRICE": "Avg_Price"})

	ytd_community_avg_sale_price: DataFrame = pd.concat([community_sold_count, ytd_avg_price], axis=1)
	del ytd_community_avg_sale_price['SALE_PRICE']
		
	ytd_community_avg_sale_price['Avg_Price']: str = ytd_community_avg_sale_price['Avg_Price'].apply(format_price)
	ytd_community_avg_sale_price.reset_index(inplace=True)
	ytd_community_avg_sale_price.to_csv(f"{my_secrets.csv_finance_path}ytd_community_avg_sale_price.csv")
