...
def tables():
    ...
    communities_tbl = communities_tbl_insp.has_table(
        COMMUNITY_TOTALS, schema=f"{DB_NAME}"
    )
    ...
    historical_sales_tbl = historical_sales_insp.has_table(
    ...
    if not parcels_tbl:
        ...
        try:
            parcel_constants = pd.read_csv(
                f"../input/{PARCELS}.csv", index_col=0, header=0, skiprows=None
            )
        except IOError as e:
            logger.error(e)
            return False
        ...
        with engine.connect() as conn, conn.begin():
            try:
                ...
                logger.warning(
                    f"Table: {PARCELS} did not exist and has been created and seeded"
                )
    ...
    if not communities_tbl:
        ...
        try:
            with engine.connect() as conn, conn.begin():
                q_community_parcel_totals = conn.execute(
                    text(
            ...
            logger.warning(
                f"Table: {COMMUNITY_TOTALS} did not exist and has been created and seeded"
            )
